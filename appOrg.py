...
col1.metric("Gesamtwert (USD)", f"{total_value:,.0f}")
col2.metric("Gesamtgewicht (kg)", f"{total_weight:,.0f}")
col3.metric("Ø Preis (USD)", f"{avg_price:,.0f}" if pd.notnull(avg_price) else "–")

# Vergleich Preis/kg mit Vorjahr
...
# Formatierter Wert (Fix des Fehlers!)
ppk_value = price_per_kg if price_per_kg else 0
formatted_ppk = f"{ppk_value:,.2f}"

col4.markdown(
    f"""
    <div style="background-color:{kpi_color}; padding:16px; border-radius:10px; text-align:center;">
        <h4 style="color:white; margin-bottom:4px;">Preis pro kg (USD/kg)</h4>
        <h2 style="color:white; margin:0;">{formatted_ppk}</h2>
    </div>
    """,
    unsafe_allow_html=True
)
...
with col4_:
    ...
    if not filtered.empty:
        ...
        fig_country = px.bar(by_country, x="country_name", y="price",
                             labels={"country_name": "Land", "price": "Importwert (USD)"},
                             title="Importwert nach Ländern")
...
with col5_:
    ...
    if not filtered.empty:
        ...
        fig_product = px.bar(by_product, x="product_name", y="price",
                             labels={"product_name": "Produkt", "price": "Importwert (USD)"},
                             title="Importwert nach Produkten")
...
if not filtered.empty:
    ...
    fig_year = px.line(by_year, x="year", y="price", markers=True,
                       labels={"year": "Jahr", "price": "Importwert (USD)"},
                       title="Entwicklung des Importwerts über die Jahre")
...
fig_ppk = px.line(
    ...
    y="price_per_kg",
    markers=True,
    labels={"year": "Jahr", "price_per_kg": "Preis pro kg (USD/kg)"},
    title="Entwicklung des Preis pro kg"
)
st.plotly_chart(fig_ppk, use_container_width=True)

# ---------------------------------------------------------
# YoY Analyse (Importwert)
# ---------------------------------------------------------
st.subheader("📉 Year-over-Year Veränderung (Importwert)")

yoy_df = (
    df.groupby("year", as_index=False)["price"].sum()
    .sort_values("year")
)

yoy_df["prev_year"] = yoy_df["price"].shift(1)
yoy_df["yoy_abs"] = yoy_df["price"] - yoy_df["prev_year"]
yoy_df["yoy_pct"] = (yoy_df["yoy_abs"] / yoy_df["prev_year"]) * 100

fig_yoy = px.bar(
    yoy_df,
    x="year",
    y="yoy_pct",
    title="YoY % Veränderung des Importwerts",
    labels={"yoy_pct": "YoY (%)", "year": "Jahr"},
)
st.plotly_chart(fig_yoy, use_container_width=True)

# ---------------------------------------------------------
# Risiko-Heatmap (Abhängigkeit)
# ---------------------------------------------------------
